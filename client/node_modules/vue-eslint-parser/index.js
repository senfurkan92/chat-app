...
function getEslintScope() {
    ...
    if (!escopeCache) {
        ...
        if (!escopeCache ||
            ...
            semver.lte(escopeCache.version, escope.version)) {
            escopeCache = escope;
        }
    }
    return escopeCache;
...
function getEspreeFromEcmaVersion(ecmaVersion) {
    ...
    if (linterEspree.version != null &&
        semver.lte(espree.version, linterEspree.version)) {
        return linterEspree;
    }
    if (ecmaVersion == null) {
        return linterEspree;
    ...
    function getLinterLatestEcmaVersion() {
        if (linterEspree.latestEcmaVersion == null) {
            for (const { v, latest } of [
                ...
                if (semver.lte(v, linterEspree.version)) {
                    return latest;
                }
            }
            return 2018;
...
function getNewestEspree() {
    const linterEspree = getEspreeFromLinter();
    if (linterEspree.version == null ||
        semver.lte(linterEspree.version, espree.version)) {
        return espree;
    }
    return linterEspree;
}
...
function getScriptSetupParserOptions(parserOptions) {
    const espreeEcmaVersion = getEcmaVersionIfUseEspree(parserOptions, getDefaultEcmaVersion$1);
    return Object.assign(Object.assign({}, parserOptions), { ecmaVersion: espreeEcmaVersion });
}
function getDefaultEcmaVersion$1(def) {
    if (semver.lte("8.0.0", getEspreeFromLinter().version)) {
...
function isScriptSetup(script) {
    return script.startTag.attributes.some((attr) => !attr.directive && attr.key.name === "setup");
}
function parseScriptSetupElements(scriptSetupElement, scriptElement, code, linesAndColumns, originalParserOptions) {
    const parserOptions = getScriptSetupParserOptions(originalParserOptions);
    const scriptSetupCodeBlocks = getScriptsCodeBlocks(scriptSetupElement, scriptElement, code, linesAndColumns, parserOptions);
    if (!scriptSetupCodeBlocks) {
        return parseScriptFragment("", simpleOffsetLocationCalculator(scriptSetupElement.startTag.range[1], linesAndColumns), parserOptions);
    }
    ...
    let result;
    try {
        result = parseScript(scriptSetupCodeBlocks.codeBlocks.code, parserOptions);
    }
    catch (err) {
    ...
    if (result.ast.end != null) {
        result.ast.end = [scriptSetupElement, scriptElement].reduce((end, node) => {
            ...
            return Math.max(end, textNode != null && textNode.type === "VText"
                ? textNode.range[1]
                : (_b = (_a = node.endTag) === null || _a === void 0 ? void 0 : _a.range[1]) !== null && _b !== void 0 ? _b : node.range[1]);
        }, 0);
    }
    return result;
}
function getScriptsCodeBlocks(scriptSetupElement, scriptElement, sfcCode, linesAndColumns, parserOptions) {
    ...
    return {
        codeBlocks,
        scriptSetupImportRange: scriptSetupCodeBlocks.scriptSetupImportRange && [
...
function getScriptSetupCodeBlocks(node, sfcCode, linesAndColumns, parserOptions) {
    ...
    try {
        ...
        const perr = ParseError.normalize(err);
        if (perr) {
            fixErrorLocation(perr, simpleOffsetLocationCalculator(scriptStartOffset, linesAndColumns));
            throw perr;
        }
        throw err;
    }
    ...
    for (const body of ast.body) {
        if (body.type === "ImportDeclaration") {
            ...
            if (astOffset < start) {
                statementCodeBlocks.append(scriptCode.slice(astOffset, start), scriptStartOffset + astOffset);
                statementCodeBlocks.appendSplitPunctuators(";");
            }
            importCodeBlocks.append(scriptCode.slice(start, end), scriptStartOffset + start);
            importCodeBlocks.appendSplitPunctuators(";");
            astOffset = end;
        }
    }
    if (astOffset < scriptEndOffset) {
        statementCodeBlocks.append(scriptCode.slice(astOffset, scriptEndOffset), scriptStartOffset + astOffset);
    }
    const scriptSetupImportRange = [
        0,
...
function remapAST(result, { scriptSetupImportRange, scriptSetupBlockRange, codeBlocks, }) {
    ...
    for (let index = result.ast.body.length - 1; index >= 0; index--) {
        ...
        if (body.type === "BlockStatement") {
            if (scriptSetupBlockRange[0] <= body.range[0] &&
                ...
                result.ast.body.splice(index, 1, ...body.body.filter((b) => !isSplitPunctuatorsEmptyStatement(b)));
            }
        }
...
function simpleOffsetLocationCalculator(offset, linesAndColumns) {
    return {
        getFixOffset() {
            return offset;
        },
...
function parseScriptElement(node, globalLocationCalculator, originalParserOptions) {
    const parserOptions = isScriptSetup(node)
        ? getScriptSetupParserOptions(originalParserOptions)
        : Object.assign(Object.assign({}, originalParserOptions), { ecmaVersion: originalParserOptions.ecmaVersion || 2017 });
    const text = node.children[0];
    const { code, offset } = text != null && text.type === "VText"
...
class Parser {
    ...
    getTagName(startTagOrElement) {
        return getTagName$1(startTagOrElement, this.isSFC);
    }
...
function parseForESLint(code, options) {
    ...
    const optionsWithEcmaVersion = Object.assign(Object.assign({}, options), { ecmaVersion: options.ecmaVersion || 2017 });
    let result;
    let document;
    ...
    if (!isVueFile(code, options)) {
        ...
        const tokenizer = new Tokenizer(code, optionsWithEcmaVersion);
        const rootAST = new Parser(tokenizer, optionsWithEcmaVersion).parse();
        ...
        if (skipParsingScript || !scripts.length) {
            ...
            result = parseScriptSetupElements(scriptSetup, scripts.find((e) => e !== scriptSetup), code, new LinesAndColumns(tokenizer.lineTerminators), options);
        }
        else {
